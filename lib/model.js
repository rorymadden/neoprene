...
Model.prototype.create = function(uid, options, callback){
  ...
  if(options.relationship){
    ...
    if(options.relationship.indexField!== '_id' || options.eventNodes.relationshipNode || options.eventNodes.user){
      query += ' MATCH';
      if(options.eventNodes.user) {
        query += ' user-[userLatestRel:LATEST_EVENT]->(userLatestEvent),';
      }
    }
    if(options.relationship.indexField!== '_id' || options.eventNodes.relationshipNode) {
      query+= ' (relNode';
      ...
      if(options.eventNodes.relationshipNode) query += ')-[relNodeLatestRel:LATEST_EVENT]->(relNodeLatestEvent)';
      else query += ')';
...
Model.prototype.update = function(updates, uid, options, callback){
  ...
  for(var key in updates){
    ...
    if(typeof this[key] !== 'undefined' && key !== 'password' ){
    ...
    if(updates[key] !== null && typeof updates[key] !== 'undefined'){
      // TODO - add escape for other fields
      if(key !== 'password') params[key+'_NEW'] = updates[key];

      setQuery += ' SET node.'+ key +' = {'+key+ '_NEW}';
      if(key !== 'password') eventNodeText += ', ' + key + '_NEW: {'+key+'_NEW}';
    }
...
Model.prototype['delete'] = function (force, callback) {
  ...
  if (!this._id) {
    ...
    // and then the node
    if (force) {
      // turn force into a count to retry the delete if there is an issue
