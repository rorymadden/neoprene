...
/**
 * Find one or more indexed Relationships by index, key and value. Returns
 * error if not found or the appropriate loaded Model
 * @param  {String}   index
 * @param  {String}   key
 * @param  {String}   value
 * @param  {Function} callback
 ...
 * @api public
 */
Neoprene.prototype.getIndexedRelationships = function(index, key, value, callback){
  return this.getIndexed(index, key, value, 'relationship', callback);
};

/**
 * Find one or more indexed Nodes by index, key and value. Returns
 * error if not found or the appropriate loaded Model
 * @param  {String}   index
 * @param  {String}   key
 * @param  {String}   value
 ...
 * @api public
 */
Neoprene.prototype.getIndexedNodes = function(index, key, value, callback){
  return this.getIndexed(index, key, value, 'node', callback);
};

/**
 * This is used by getIndexedNodes and getIndexedRelationships.
 * Fetch array of nodes or relationships which match an index key and value.
 * If no objects exist an empty array will be returned.
 *
 * NOTE: This will be moved to PropertyContainer.getIndexed once
 * rewrite is complete
 *
 * @param  {String}   index
 * @param  {String}   key
 * @param  {String}   value
 * @param  {Function} callback
 * @return {Array}             The array of matched nodes / relationships.
 * @api    private
 */
Neoprene.prototype.getIndexed = function(index, key, value, objectType, callback) {
  // validate that index, key and value exist
  if (!value || typeof(value) === 'function') {
    callback = value ? value : key ? key : index ? index : type;
    process.nextTick(function() {
      return callback(new NeopreneError('Invalid index request'), null);
    });
  }
  else {
    var self = this;
    // encode key and value so that they can be included in REST API url
    key = encodeURIComponent(key);
    var val = encodeURIComponent(value);
    var url = this.url + '/db/data/index/' + objectType + '/' + index +
              '/' + key + '/' + val;
    ...
    request
      ...
      .set('X-Stream', true)
      .end(function(res) {
        // REST API returns 200 on success
        if (res.status !== 200) {
          if (res.status === 404) {
            return callback(new NeopreneError('Invalid Index'), null);
          }
          return callback(new NeopreneError(res.body.message), null);
        }
        // map all of the items returned into their appropriate objects
        // e.g. Node or Relationship
        async.map(res.body, function(node, callback) {
          callback(null, utils.loadObject(node, self));
        }, function(err, nodes) {
 ...
 * @api public
 */
Neoprene.prototype.getIndexedNode = function(index, key, value, callback){
  this.getIndexedNodes(index, key, value, function(err, results){
    if(err){
      return callback(err, null);
    }
    if(results.length > 1){
      return callback(new NeopreneError('More than one node exists'), null);
    }
 ...
 * @api public
 */
Neoprene.prototype.getIndexedRelationship = function(index, key, value, callback){
  this.getIndexedRelationships(index, key, value, function(err, results){
    if(err){
      return callback(err, null);
    }
    if(results.length > 1){
      return callback(new NeopreneError('More than one relationship exists'),
        null);
    }
    return callback(null, results[0]);
  });
 ...
 * @api    public
 */
Neoprene.prototype.queryRelationshipIndex = function(index, query, callback){
  return this.queryIndex(index, query, 'relationship', callback);
};
/**
 ...
 * @api    public
 */
Neoprene.prototype.queryNodeIndex = function(index, query, callback){
  return this.queryIndex(index, query, 'node', callback);
};

/**
 * Run a Lucene query against a node/rel index. If no nodes/rels exist
 * an empty array is returned.
 *
 * NOTE: Neo4j currently uses Lucene 3.5
 *
 ...
 * @param  {String}   objectType  'node' or 'relationship'.
 * @param  {Function} callback
 * @return {Array}                The array of matched nodes/rels or empty array.
 * @api    private
 */
Neoprene.prototype.queryIndex = function(index, query, objectType, callback) {
  // validate that the correct number of arguments are present
  if (!query || typeof(query) === 'function') {
    callback = query ? query : index;
    query = null;
  }
  if (!query) {
    process.nextTick(function() {
      return callback(new NeopreneError('Need to include index and query'), null);
    });
  }
  else {
    var self = this;
    // encode the query for the REST API url
    query = encodeURIComponent(query);
    var url = this.url + '/db/data/index/' +  objectType + '/' + index + '?query=' + query;
    ...
    request
      ...
      .end(function(res) {
        ...
        // map the objects returned to nodes
        async.map(res.body, function(node, callback) {
          callback(null, utils.loadObject(node, self));
        }, function(err, nodes) {
          if (err) return callback(new NeopreneError(err), null);
