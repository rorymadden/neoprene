...
Model.prototype.save = function(relationship, callback) {
  ...
  // else this is a new node / relationship
    ...
    if(relationship){
      ...
      query += 'CREATE (newnode:' + relationship.nodeLabel + ' {newNode}), '
 ...
 * @api    private
 */
Model.prototype._getRelationships = function(direction, type, label, callback) {
  ...
  var url = self.db + '/db/data/cypher';
  var query = 'START n=node(' + self._id + ') MATCH (n)';
  ...
  request
    ...
    .end(function(res) {
      ...
      if (res.status === 200) {
        ...
        for(var i= 0; i< len; i++){
          var direction
          if(data[i][0].start === self.self) direction = 'out';
          else direction = 'in';
          var type = data[i][0].type;

          var rel = utils.loadObject(data[i][0], self.base);
          for(var key in rel._doc){
            if(key !== '_id') {
              if(!rel._doc.data) rel._doc.data = {};
              rel._doc.data[key] = rel._doc[key];
              delete rel._doc[key]
            }
          }
          rel._doc.direction = direction;
          rel._doc.type = type;

          rels.push(rel)
          nodes.push(utils.loadObject(data[i][1], self.base))
        }
 ...
 * @api    public
 */
Model.prototype.getAllRelationships = function(type, label, callback) {
  return this._getRelationships('all', type, label, callback);
};
 ...
 * @api    public
 */
Model.prototype.getOutgoingRelationships = function(type, label, callback) {
  return this._getRelationships('out', type, label, callback);
};
 ...
 * @api    public
 */
Model.prototype.getIncomingRelationships = function(type, label, callback) {
  return this._getRelationships('in', type, label, callback);
};
