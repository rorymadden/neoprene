...
Model.prototype._createRelationship = function(from, to, type, data, callback) {
  ...
  if (typeof(type) === 'function' || !from.self || !to.self) {
    ...
    // var self = this;
    // var query = 'START from = node(' + from._id + '), to = node(' + to._id + ') CREATE from-[r:' + type + ' {data}]->to RETURN r';
    // var cypherUrl = this.db + '/db/data/cypher';
    // var cypherQuery = { query: query, params: {data: data} };
    ...
    //     // REST API returns 200
    //     if (res.status === 200) {
    //       return callback(null, utils.loadObject(res.body.data[0], self.base));
    //     }
