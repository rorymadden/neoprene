...
GraphObject.prototype.__proto__ = EventEmitter.prototype;
...
GraphObject.prototype._set = function (path, constructing, schema, val, priorVal) {
  ...
  // update the value
  this._doc[path] = val;
  // console.log('This doc '+JSON.stringify(this._doc))
};
...
GraphObject.prototype._registerHooks = function _registerHooks () {
  ...
  // }).pre('save', function checkUniqueIndexes(next){
  //   return this.validateUniqueIndexes(next)
  // }).post('save', function applyIndexes(next){
  //   return this.applyIndexes(next);
  });
...
function applyGetters (self, json, type, options) {
  ...
  while (i--) {
    path = paths[i];
    // if(type === 'virtuals') console.log(self.get(path))
    json[path] = clone(self.get(path), options);
    // if(type === 'virtuals') console.log(json[path]);
  }
